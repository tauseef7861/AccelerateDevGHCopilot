...
public class ConsoleApp
{
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        {
            Console.Write("Book title cannot be empty. Please enter a book title to search for: ");
            bookTitle = Console.ReadLine();
        }

        // Search for book with case-insensitive match using FirstOrDefault
        Book? foundBook = _jsonData.Books?.FirstOrDefault(b => 
            b.Title.Contains(bookTitle, StringComparison.OrdinalIgnoreCase));

        if (foundBook == null)
        {
            Console.WriteLine($"Book title \"{bookTitle}\" not found in the library catalog.");
            return ConsoleState.PatronDetails;
        }

        // Get the book item for this book
        BookItem? bookItem = _jsonData.BookItems?.FirstOrDefault(bi => bi.BookId == foundBook.Id);

        if (bookItem == null)
        {
            Console.WriteLine($"No copies of \"{foundBook.Title}\" available in the library.");
            return ConsoleState.PatronDetails;
        }

        // Check for active loan (ReturnDate == null)
        Loan? activeLoan = _jsonData.Loans?.FirstOrDefault(l => 
